...
async function ensurePlatformTenants() {
  ...
  try {
    ...
    console.log(`✅ Platform tenant ensured (${isTradingService ? 'trade' : 'core'}=${activeTenantId})`);
  } catch (error: any) {
...
// Authentication endpoints (Extracted to Identity Service)
...
// Trading & Syndication (Extracted to Market Service)
app.use('/api/launchpad', proxyService(MARKET_SERVICE_URL));
app.use('/api/syndicates', authenticateToken, proxyService(MARKET_SERVICE_URL));
app.use('/api/syndicate-tokens', authenticateToken, proxyService(MARKET_SERVICE_URL));
app.use('/api/secondary-trading', authenticateToken, proxyService(MARKET_SERVICE_URL));
...
// Microservice Proxies
app.use('/api/advisory', authenticateToken, proxyService(ADVISORY_SERVICE_URL));
app.use('/api/advisory-services', authenticateToken, proxyService(ADVISORY_SERVICE_URL));
app.use('/api/advisors', authenticateToken, proxyService(ADVISORY_SERVICE_URL));
app.use('/api/dataroom', authenticateToken, proxyService(DOCUMENT_SERVICE_URL));
app.use('/api/document', authenticateToken, proxyService(DOCUMENT_SERVICE_URL));
app.use('/api/documents', authenticateToken, proxyService(DOCUMENT_SERVICE_URL));
app.use('/api/audit', authenticateToken, auditRoutes);
app.use('/api/payments', authenticateToken, proxyService(FUNDING_SERVICE_URL));
app.use('/api/cashflow', authenticateToken, proxyService(FUNDING_SERVICE_URL));
app.use('/api/disputes', authenticateToken, disputeRoutes);
app.use('/api/escrow', authenticateToken, proxyService(FUNDING_SERVICE_URL));
...
app.use('/api/admin/advisor-ops', authenticateToken, proxyService(ADVISORY_SERVICE_URL));
